...
async function run() {
  ...
  const excusedEmails = fs.readFileSync(excusedPath, 'utf8')
    .trim().split('\n').filter(Boolean).map(normalizeEmail);
  
  console.log('Excused emails to mark:', excusedEmails.length);
  
  // Load 22 May CSV to get student data for excused ones
  ...
  const rows = parseCsv(fs.readFileSync(csv22Path, 'utf8').replace(/^\uFEFF/, ''));
  
  const excusedSet = new Set(excusedEmails);
  const excusedStudents = rows.slice(1)
    .filter(r => excusedSet.has(normalizeEmail(r[1])))
    .map(r => ({
  ...
  console.log('Excused students loaded:', excusedStudents.length);
  
  await Student.deleteMany({ status: 'excused' });
  await Student.insertMany(excusedStudents);
  console.log('Inserted', excusedStudents.length, 'excused students into DB');
